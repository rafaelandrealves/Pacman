...
BONUS_POS		WORD		0620h		;posição do ponto bonus
...
Perdi    STR '***--------Црвена звезда----------***',FIM_TEXTO
...
modo_inteligente_xx:		NOP
							...
							PUSH	R1;posição dos monstros (yyxx)
							...
							MOV		R2, M[PacMan]
							AND		R2, 00FFh
	ciclo_monstros_xx:		MOV		R1, M[R5 + Monsters]
							MOV		R4, R1
							MOV		R3, R1
							AND		R4, 00FFh
							...
							BR.P	move_right
							BR.N	move_left
							CALL	modo_inteligente_yy
							BR		fim_modo_int_xx;alterar!!!
...
modo_inteligente_yy:		NOP
							...
							MOV		R2, M[PacMan]
							SHR		R2, 8
	ciclo_monstros_yy:		MOV		R1, M[R5 + Monsters]
							MOV		R4, R1
							MOV		R3, R1
							SHR		R4, 8
							CMP		R2, R4 ;se R2, for maior que R1 o pacman encontra-se à direita dos monstros
...
;===============================================================================
;Reset_Labi
;	Rotina que cria uma copia do tabuleiro original
;===============================================================================
Reset_Labi:		NOP
				PUSH	R1
				PUSH	R2
...
NoUpDateDisplay:	NOP
          ...
          DEC 	R5
          CMP 	R5,R0
          BR.NZ	MOVIM; caso tenha já preenchido uma das velocidades, vai chamar o movimento dos monstros
          CALL	modo_de_jogo

					; só corre de segundo a segundo
					;faz update do display hexa
	MOVIM:	CALL	UpDate_Display
	...
	SALTA:  CMP 	R5,0; vê se o tempo atual é igual a 90,60 ou 30, para variar a velocidade
					JMP.NZ 	NoUpDateDisplay
	MOVI:		MOV 	R4,005Ah
					CMP 	R2,R4
					BR.NZ 	STEP1
					INC 	R6
...
Em_Pausa:	MOV	R1, M[Pausa]
			...
			MOV		M[PORTMASCARA_INT], R1
      CMP	R1, 1
			BR.NZ	Em_Pausa	;se no endereço estiver a um, ele continua em pausa
			MOV		R1, 9155h
			MOV		M[PORTMASCARA_INT], R1
      CMP   R3,R0
      JMP.P inicio
